...
def generiere_bestellschein_bytes(klassenstufe, schuljahr, lehrbuecher, 
    ...
    if total_ah > 0:
        doc.add_paragraph("Arbeitshefte/Lektüre").runs[0].font.bold = True
...
def main_app():
    ...
    with tab_bestellschein:
        ...
        logo_bytes = None
        try:
            with open("/mnt/user-data/uploads/ifxe4a2ae0bda55cdd7_eduresized_LBS_-_Ginkgo-Blatt.jpg", "rb") as f:
                logo_bytes = f.read()
        except:
            st.warning("⚠️ Logo nicht gefunden - Bestellschein wird ohne Logo erstellt")
        ...
        with col1:
            klassenstufe = st.selectbox("Klassenstufe", 
                                        ["5", "6", "7", "8", "9", "10", "11", "12"])
            schuljahr = st.text_input("Schuljahr", value="2025/2026")
            ...
            if not ausgewaehlte_buecher and not arbeitshefte_text.strip():
                ...
                with st.spinner("Erstelle Bestellschein..."):
                    ...
                    for b in ausgewaehlte_buecher:
                        preis_str = f"{b.get('preis', 0):.2f} €" if b.get('preis') else ""
                    ...
                    if arbeitshefte_text.strip():
                        ...
                        for isbn in ah_isbns:
                            ...
                            if buch:
                                ...
                                # Platzhalter wenn nicht im Bestand
                                arbeitshefte_data.append({
                                    "fach": "",
                                    "titel": f"ISBN: {isbn}",
                                    "verlag": "",
