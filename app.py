...
def buch_formular(db, existing: dict | None = None):
    ...
    with st.form(key=f"buch_form_{form_id}"):
        ...
        with c1:
            ...
            # Auto-Lookup aus Katalog wenn ISBN eingegeben und Titel leer
            if is_new and isbn_input and not existing.get("titel"):
                ...
                if katalog_data:
                    ...
                    st.success(f"✅ Daten aus Katalog geladen: {existing['titel'][:50]}")
            
            isbn = isbn_input
            titel  = st.text_input("Titel *", value=existing["titel"],
                                   help="Wird automatisch aus Katalog geladen wenn ISBN bekannt")
...
def main_app():
    ...
    with tab_liste:
        ...
        if df_view.empty:
            ...
            buch_optionen_quick = {
                f"{b['titel']} (ISBN: {b['isbn']})": b['isbn']
                for b in buecher
            }
            selected_quick = st.selectbox(
                "Buch zum Bearbeiten auswählen",
                [""] + list(buch_optionen_quick.keys()),
                key="quick_select"
            )
            if selected_quick and st.button("📝 Bearbeiten", key="quick_edit"):
                st.session_state["edit_isbn"] = buch_optionen_quick[selected_quick]
                st.info(f"✅ Buch ausgewählt! Wechsle zum Tab '✏️ Buch bearbeiten' um es zu bearbeiten.")
                st.rerun()
    ...
    with tab_bestellschein:
        ...
        # Versuche Logo von GitHub/lokal zu laden
        logo_pfade = ["logo.jpg", "./logo.jpg", "logo.png", "./logo.png"]
        for pfad in logo_pfade:
            try:
                with open(pfad, "rb") as f:
                    logo_bytes = f.read()
                    st.success(f"✅ Logo geladen: {pfad}")
                    break
            except:
                continue
        ...
        if not logo_bytes:
            logo_file = st.file_uploader("📷 Logo hochladen (optional)", 
                                          type=["jpg", "jpeg", "png"],
        ...
        if klassenstufe == "5":
            ...
            weiterfuehrung_text = st.text_area(
                "ISBNs weitergeführter Bücher (eine pro Zeile):",
                height=100
            )
            vorjahr_klasse = str(int(klassenstufe) - 1)
                ...
                with st.spinner("Erstelle Bestellschein..."):
                    ...
                    for b in ausgewaehlte_buecher:
                        ...
                        lehrbuecher_data.append({
                            ...
                            "isbn": b.get("isbn", ""),
                            "preis": preis_str,
                            "klassensatz": False  # TODO: Aus DB holen wenn Feld existiert
                        })
                    ...
                    weiterfuehrung_data = []
                    if klassenstufe != "5" and 'weiterfuehrung_text' in locals() and weiterfuehrung_text.strip():
                        wf_isbns = [isbn.strip() for isbn in weiterfuehrung_text.strip().split("\n") if isbn.strip()]
                        for isbn in wf_isbns:
                            buch = next((b for b in buecher if b["isbn"] == isbn), None)
                            katalog_data = hole_buch_aus_katalog(isbn)
