...
FACH_REIHENFOLGE = ["D", "Ma", "Eng", "MNT", "Geo", "Bio", "Ch", "Ph", "Sn", "Eth", "Rel", "WR", "DaZ"]
...
def generiere_bestellschein_bytes(klassenstufe, schuljahr, lehrbuecher, 
    ...
    if lehrbuecher:
        ...
        table = doc.add_table(rows=1 + len(lehrbuecher_sortiert), cols=9)
    ...
    if total_wf > 0:
        ...
        for idx, wf in enumerate(weiterfuehrung_sortiert):
            ...
            for cell in row.cells[:4]:  # Nur die ersten 4 Spalten
                if cell.paragraphs and cell.paragraphs[0].runs:
                    cell.paragraphs[0].runs[0].font.size = Pt(8)
            for cell in row.cells:
                shading = OxmlElement('w:shd')
