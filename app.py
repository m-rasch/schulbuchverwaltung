...
def buch_formular(db, existing: dict | None = None):
    ...
    with st.form(key=f"buch_form_{form_id}"):
        ...
        c1, c2 = st.columns(2)
        with c1:
            col_isbn, col_lookup = st.columns([3, 1])
            ...
            with col_lookup:
                st.write("")  # Spacing
                if st.button("🔍 Daten holen", help="Lade Titel, Verlag, Fach aus Katalog"):
                    if isbn_input:
                        ...
                        if katalog_data:
                            ...
                            if katalog_data.get("preis"):
                                try:
                                    existing["preis"] = float(katalog_data.get("preis", "").replace("€", "").strip())
                                except:
                                    pass
                            st.success("✅ Daten aus Katalog geladen!")
                            st.rerun()
                        else:
                            st.warning("⚠️ ISBN nicht im Katalog gefunden - bitte manuell eintragen")
                    else:
                        st.warning("⚠️ Bitte erst ISBN eingeben")
            
            isbn = isbn_input
            titel  = st.text_input("Titel *", value=existing.get("titel", ""),
                                   help="Klicke '🔍 Daten holen' um automatisch zu laden")
