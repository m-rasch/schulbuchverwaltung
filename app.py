...
def main_app():
    ...
    with st.sidebar:
        ...
        if not df.empty:
            if st.button("🔄 Preise aktualisieren", use_container_width=True, 
                        help="Holt aktuelle Preise von Google Books API"):
                with st.spinner("Preise werden aktualisiert..."):
                    ...
                    if result["erfolg"] > 0:
                        ...
                        st.warning(f"⚠️ Keine Preise gefunden. Versuche es später nochmal oder trage Preise manuell ein.")
                    
                    if result["fehler"] > 0:
                        st.info(f"ℹ️ {result['fehler']} Bücher: Kein Preis verfügbar (manuell eintragen)")
    ...
    with tab_liste:
        ...
        if df_view.empty:
            ...
            # Preis formatieren
            display_df["preis"] = display_df["preis"].apply(
