...
ALLE_FAECHER = [
    ...
    "Deutsch als Zweitsprache","Sonstiges",
]
...
def export_schuelerzahlen_txt(db, schuljahr=None) -> str:
    ...
    if not schuljahr:
        # NÄCHSTES Schuljahr (Planungsjahr)
        aktuelles_jahr = datetime.now().year
        aktueller_monat = datetime.now().month
        if aktueller_monat >= 8:
            naechstes_start = aktuelles_jahr + 1
        else:
            naechstes_start = aktuelles_jahr
        naechstes_end = naechstes_start + 1
        schuljahr = f"{naechstes_start}/{naechstes_end}"
    
    try:
        schueler_ref = db.collection('schuelerzahlen').document(schuljahr.replace('/','_'))
        ...
        if schueler_doc.exists:
            ...
            # Zeige Modi
            sn_mode = schueler_data.get("spanisch_mode", "jahrgang")
            fr_mode = schueler_data.get("franzoesisch_mode", "jahrgang")
            daz_mode = schueler_data.get("daz_mode", "jahrgang")
            
            if sn_mode == "gesamt":
                lines.append(f"Spanisch (Gesamt): {schueler_data.get('spanisch_gesamt_total', 0)} Schüler")
            if fr_mode == "gesamt":
                lines.append(f"Französisch (Gesamt): {schueler_data.get('franzoesisch_gesamt_total', 0)} Schüler")
            if daz_mode == "gesamt":
                lines.append(f"DaZ (Gesamt): {schueler_data.get('daz_gesamt_total', 0)} Schüler")
            
            if sn_mode == "gesamt" or fr_mode == "gesamt" or daz_mode == "gesamt":
                lines.append("-" * 120)
            
            jahrgaenge = ["5", "6", "7", "8", "9", "10", "10g", "11", "12"]
            for jg in jahrgaenge:
                gesamt = schueler_data.get(f"{jg}_gesamt", 0)
                sn = schueler_data.get(f"{jg}_spanisch", 0) if sn_mode == "jahrgang" else 0
                fr = schueler_data.get(f"{jg}_franzoesisch", 0) if fr_mode == "jahrgang" else 0
                rel = schueler_data.get(f"{jg}_religion", 0)
                daz = schueler_data.get(f"{jg}_daz", 0) if daz_mode == "jahrgang" else 0
                
                if gesamt > 0 or sn > 0 or fr > 0 or rel > 0 or daz > 0:
                    line = f"Klasse {jg:>3}: {gesamt:>3} Schüler"
                    if sn > 0:
                        line += f" (Sn: {sn:>2}"
                    if fr > 0:
                        if sn > 0:
...
def hole_schuelerzahl_fuer_buch(db, fach, klasse, schuljahr=None):
    ...
    if not schuljahr:
        # NÄCHSTES Schuljahr berechnen (wir planen für nächstes Jahr)
        aktuelles_jahr = datetime.now().year
        aktueller_monat = datetime.now().month
        if aktueller_monat >= 8:
            naechstes_start = aktuelles_jahr + 1
    ...
    try:
        ...
        # Prüfe ob Spezialfach und welcher Modus
        if "Spanisch" in fach or "Sn" in fach:
            modus = schueler_data.get("spanisch_mode", "jahrgang")
            if modus == "gesamt":
                return schueler_data.get("spanisch_gesamt_total", 0)
            else:
                return schueler_data.get(f"{jahrgang}_spanisch", 0)
        elif "Französisch" in fach or "Fr" in fach:
            modus = schueler_data.get("franzoesisch_mode", "jahrgang")
            if modus == "gesamt":
                return schueler_data.get("franzoesisch_gesamt_total", 0)
            else:
                return schueler_data.get(f"{jahrgang}_franzoesisch", 0)
        elif "DaZ" in fach or "Deutsch als Zweitsprache" in fach:
            modus = schueler_data.get("daz_mode", "jahrgang")
            if modus == "gesamt":
                return schueler_data.get("daz_gesamt_total", 0)
            else:
...
def main_app():
    ...
    with tab_schueler:
        ...
        # Für Planung immer das NÄCHSTE Schuljahr als Default
        # Vor August: nächstes = aktuelles_jahr / aktuelles_jahr+1
        # Ab August: nächstes = aktuelles_jahr+1 / aktuelles_jahr+2
        if aktueller_monat >= 8:
            naechstes_schuljahr_start = aktuelles_jahr + 1
        else:
            naechstes_schuljahr_start = aktuelles_jahr
        naechstes_schuljahr_end = naechstes_schuljahr_start + 1
        schuljahr_default = f"{naechstes_schuljahr_start}/{naechstes_schuljahr_end}"
        
        # Schuljahr mit Session State handhaben
        if "schuljahr_input" not in st.session_state:
            st.session_state.schuljahr_input = schuljahr_default
        
        col_sj, col_btn = st.columns([3, 1])
        with col_sj:
            schuljahr_input = st.text_input(
                "📅 Schuljahr", 
                value=st.session_state.schuljahr_input,
                key="schuljahr_text_input"
            )
            st.session_state.schuljahr_input = schuljahr_input if schuljahr_input else schuljahr_default
        
        # Nutze gespeicherten Wert
        schuljahr_input = st.session_state.schuljahr_input
        
        # Validiere Schuljahr-Input
        if not schuljahr_input or "/" not in schuljahr_input:
            schuljahr_input = schuljahr_default
            st.session_state.schuljahr_input = schuljahr_default
        
        # Firebase erlaubt keinen "/" im Dokument-Namen → ersetze durch "_"
        schuljahr_firebase_id = schuljahr_input.replace("/", "_")
        
        # Initialisiere neue_daten SOFORT
        neue_daten = {}
        
        # Definiere Referenz mit bereinigtem Namen
        schueler_ref = db.collection('schuelerzahlen').document(schuljahr_firebase_id)
        ...
        # Modi für Spanisch, Französisch und DaZ
        st.markdown("#### Eingabe-Modus für Fächer mit variablen Schülerzahlen")
        
        col_sn_mode, col_fr_mode, col_daz_mode = st.columns(3)
        
        with col_sn_mode:
            sn_mode = st.radio(
                "🇪🇸 Spanisch:",
                ["Pro Jahrgang", "Nur Gesamt-Zahl"],
                index=0 if schueler_data.get("spanisch_mode", "jahrgang") == "jahrgang" else 1,
                key="spanisch_mode_radio",
                horizontal=True
            )
            neue_daten["spanisch_mode"] = "jahrgang" if sn_mode == "Pro Jahrgang" else "gesamt"
            
            if sn_mode == "Nur Gesamt-Zahl":
                sn_gesamt = st.number_input(
                    "Spanisch gesamt (alle Jahrgänge):",
                    min_value=0,
                    value=schueler_data.get("spanisch_gesamt_total", 0),
                    key="spanisch_gesamt_total"
                )
                neue_daten["spanisch_gesamt_total"] = sn_gesamt
        
        with col_fr_mode:
            fr_mode = st.radio(
                "🇫🇷 Französisch:",
                ["Pro Jahrgang", "Nur Gesamt-Zahl"],
                index=0 if schueler_data.get("franzoesisch_mode", "jahrgang") == "jahrgang" else 1,
                key="franzoesisch_mode_radio",
                horizontal=True
            )
            neue_daten["franzoesisch_mode"] = "jahrgang" if fr_mode == "Pro Jahrgang" else "gesamt"
            
            if fr_mode == "Nur Gesamt-Zahl":
                fr_gesamt = st.number_input(
                    "Französisch gesamt (alle Jahrgänge):",
                    min_value=0,
                    value=schueler_data.get("franzoesisch_gesamt_total", 0),
                    key="franzoesisch_gesamt_total"
                )
                neue_daten["franzoesisch_gesamt_total"] = fr_gesamt
        
        with col_daz_mode:
            daz_mode = st.radio(
                "🌍 DaZ:",
                ["Pro Jahrgang", "Nur Gesamt-Zahl"],
                index=0 if schueler_data.get("daz_mode", "jahrgang") == "jahrgang" else 1,
                key="daz_mode_radio",
                horizontal=True
            )
            neue_daten["daz_mode"] = "jahrgang" if daz_mode == "Pro Jahrgang" else "gesamt"
            
            if daz_mode == "Nur Gesamt-Zahl":
                daz_gesamt = st.number_input(
                    "DaZ gesamt (alle Jahrgänge):",
                    min_value=0,
                    value=schueler_data.get("daz_gesamt_total", 0),
                    key="daz_gesamt_total"
                )
                neue_daten["daz_gesamt_total"] = daz_gesamt
        ...
        for jg in jahrgaenge:
            ...
            # Dynamische Spalten je nach Modus
            num_cols = 2  # Gesamt + Religion (immer)
            if sn_mode == "Pro Jahrgang":
                num_cols += 1
            if fr_mode == "Pro Jahrgang":
                num_cols += 1
            if daz_mode == "Pro Jahrgang":
                num_cols += 1
            
            cols = st.columns(num_cols)
            idx = 0
            
            # Gesamt (immer anzeigen)
            with cols[idx]:
            ...
            # Spanisch (nur wenn Jahrgangs-Modus)
            if sn_mode == "Pro Jahrgang":
                with cols[idx]:
            ...
            # Französisch (nur wenn Jahrgangs-Modus)
            if fr_mode == "Pro Jahrgang":
                with cols[idx]:
                    franzoesisch = st.number_input(
                        f"Französisch",
                        min_value=0,
                        value=schueler_data.get(f"{jg}_franzoesisch", 0),
                        key=f"schueler_{jg}_franzoesisch"
                    )
                    neue_daten[f"{jg}_franzoesisch"] = franzoesisch
                idx += 1
            
            # Religion (immer wenn mind. eins der anderen im Modus)
            with cols[idx]:
